...
- `platformpg.Queryer` has no `Query` method. The claiming `SELECT` is what adds it (Task 5).
...
- [ ] **Step 1: Write the failing test for the producer config**
...
func (c ProducerConfig) validate() error {
	...
	for i, broker := range c.Brokers {
		if strings.TrimSpace(broker) == "" {
			errs = append(errs, fmt.Errorf("Brokers[%d] is blank", i))
		}
	}
...
- [ ] **Step 5: Write the failing test for the producer's option set**
...
- [ ] **Step 9: Write the test-only broker**
...
- [ ] **Step 10: Write the integration test that proves the whole thing talks**
...
- [ ] **Step 12: Add Kafka to the compose file**
...
- [ ] **Step 13: Write the topic creation script**
...
- [ ] **Step 15: Add the relay's variables to `.env.example`**
...
- [ ] **Step 16: Bring the whole stack up by hand once**
...
- [ ] **Step 3: Write the transport envelope**
...
- [ ] **Step 4: Write the retry schedule**
...
- [ ] **Step 5: Write the relay's ports and the routing table**
...
- [ ] **Step 6: Make the producer use the header constants it now shares**
...
- [ ] **Step 8: Confirm the architecture test still holds**
...
- [ ] **Step 6: Run the full unit suite and the architecture test**
...
**Interfaces:**
- Consumes: `application.PendingMessage`, `application.PassStats`, `application.OutboxRepository`, `application.PublishWork`, `application.PublishUnitOfWork`, `platformpg.Queryer`, `platformpg.WithTx`, `pgtest`.
...
- [ ] **Step 1: Add `Query` to the Queryer**

`internal/platform/postgres/queryer.go` — add one method to the interface:

```go
	Query(ctx context.Context, sql string, args ...any) (pgx.Rows, error)
```

and extend the doc comment on `Queryer` with:

```go
// Query is here for the relay's claim: SELECT … FOR UPDATE SKIP LOCKED returns
// rows, and it must run on the transaction that will hold their locks. Both
// *pgxpool.Pool and pgx.Tx implement it, which is what lets one repository serve
// the producer's pool-bound purge and the relay's transaction-bound claim
// without knowing which it got (spec D6).
```

- [ ] **Step 2: Add the indexes the relay's two table-wide reads need**
...
- [ ] **Step 3: Confirm the migration applies and `migrations.Latest` moves**
...
func TestClaimPending_TwoTransactionsClaimDisjointRows(t *testing.T) {
	...
	if len(firstBatch) != 2 || len(secondBatch) != 2 {
		t.Fatalf("batches = %v and %v, want two rows each", ids(firstBatch), ids(secondBatch))
	}
	held := map[int64]bool{}
...
func ids(claimed []application.PendingMessage) []int64 {
...
- [ ] **Step 6: Write the relay's half of the repository**
...
- [ ] **Step 7: Write the relay's transaction boundary**
...
- [ ] **Step 1: Write the failing unit test for the loop**
...
- [ ] **Step 7: Write the delete**

Append to `internal/accounts/infrastructure/postgres/outbox_relay.go`:
...
func TestRelay_NextWait(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "a full batch",
			res:  application.PublishResult{Claimed: 100, Published: 100},
			want: 0,
			why:  "there is certainly more; sleeping on a full batch is how a backlog stops draining",
		},
		{
			name: "some work, room left",
			res:  application.PublishResult{Claimed: 4, Published: 4},
			want: 50 * time.Millisecond,
			why:  "work is arriving; stay responsive",
		},
		{
			name: "a transient failure",
			res:  application.PublishResult{Claimed: 3, Published: 1, Transient: []application.Failure{failure}},
			want: 2 * time.Second,
...
- [ ] **Step 6: Confirm the architecture test**
...
- Produces: `config.Relay`, `config.LoadRelay(getenv func(string) string) (Relay, error)`; `admin.NewServer(addr string, ready func(context.Context) error) *http.Server` and `admin.Listen(srv *http.Server) error`; and the `relay` binary.
...
func TestLoadRelay_Defaults(t *testing.T) {
	...
	cfg, err := config.LoadRelay(func(key string) string { return env[key] })
	if err != nil {
...
func TestLoadRelay_ReportsEveryProblemAtOnce(t *testing.T) {
	...
	_, err := config.LoadRelay(func(key string) string { return env[key] })
	if err == nil {
...
`config_test.go` will need `errors`, `slices`, `strings`, `log/slog` and `time`
in its imports if they are not already there.

- [ ] **Step 2: Run them and watch them fail**

Run: `go test ./internal/platform/config/`
Expected: FAIL — `undefined: config.LoadRelay`.
...
- [ ] **Step 4: Run the configuration tests and watch them pass**

Run: `go test ./internal/platform/config/ -v -run TestLoadRelay`
Expected: PASS.

- [ ] **Step 5: Give `platform/admin` the server, not just the router**
...
`main.go` needs `"log/slog"` in its imports for `wakeupFor`'s signature, and no
...
- [ ] **Step 12: Run it by hand, once, and watch a row drain**
...
- `platformpg.Queryer` now has `Query`. The inbox's `INSERT … ON CONFLICT DO
  NOTHING` needs `Exec` and its `RowsAffected`, both already there.
