...
The rule makes the poison set small, closed, and enumerable, while the environment set is open-ended. So the implementation is a **denylist and the default is retry**: `MESSAGE_TOO_LARGE`, `RECORD_LIST_TOO_LARGE`, `CORRUPT_MESSAGE`, `INVALID_RECORD`, and the two serialisation errors. (Not `INVALID_MESSAGE_SIZE`: librdkafka keeps that legacy name for code 4, which modern Kafka calls `INVALID_FETCH_SIZE` — a consumer-side error no producer sees.) Anything unrecognised waits.
